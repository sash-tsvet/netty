...
public class RPDirectByteBuf extends RPByteBuf<ByteBuffer> {

    private static final Recycler<RPDirectByteBuf> RECYCLER = new Recycler<RPDirectByteBuf>() {
        @Override
        protected RPDirectByteBuf newObject(Handle<RPDirectByteBuf> handle) {
            return new RPDirectByteBuf(handle, 0);
        }
    };

    static RPDirectByteBuf newInstance(int maxCapacity) {
        RPDirectByteBuf buf = RECYCLER.get();
        buf.reuse(maxCapacity);
        return buf;
    }

    private RPDirectByteBuf(Recycler.Handle<RPDirectByteBuf> recyclerHandle, int maxCapacity) {
