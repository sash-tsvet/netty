...
public class RPHeapByteBuf extends RPByteBuf<byte[]>  {

    private static final Recycler<RPHeapByteBuf> RECYCLER = new Recycler<RPHeapByteBuf>() {
        @Override
        protected RPHeapByteBuf newObject(Handle<RPHeapByteBuf> handle) {
            return new RPHeapByteBuf(handle, 0);
        }
    };

    static RPHeapByteBuf newInstance(int maxCapacity) {
        RPHeapByteBuf buf = RECYCLER.get();
        buf.reuse(maxCapacity);
        return buf;
    }

    RPHeapByteBuf(Recycler.Handle<? extends RPHeapByteBuf> recyclerHandle, int maxCapacity) {
